...
public class cubeFactory : MonoBehaviour
{
    ...
    private Queue<GameObject> matarialQueue = new Queue<GameObject>();
    ...
    void AddMaterial()
    {
        ...
        matarialQueue.Enqueue(newCube);

        Debug.Log($"{randomType} 원료 도착 , 큐 대기 : {matarialQueue.Count} 개");
    }

    void ProcessQueue()
    {
        if(matarialQueue.Count == 0)
        {
            Debug.Log("큐가 비어있습니다.");
            return;
        }

        GameObject cube = matarialQueue.Dequeue();
    ...
    void UpdateQueueVisua()
    {
        if (queuePoint == null) return;

        GameObject[] queueArray = matarialQueue.ToArray();
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(10, 40, 250, 20), $"원료 큐 (Queue) : {matarialQueue.Count} 개 대기");
        ...
        GUI.Label(new Rect(10, 120, 250, 20), $"완제품 (Dick) : {products[ProductType.Chair]} 개");
